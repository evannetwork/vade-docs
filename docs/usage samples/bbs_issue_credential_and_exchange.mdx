...
# BBS+ Request Credential

This example is an 'easy steps' scenario of [`issue credential protocol`], here we start with the (future) holder of a credential **requesting a credential** from an issuer.

## Exchanging information and using protocols in VADE by combining plugins

The usage sample on this page follows a story where Alice can issue credentials and Bob wants to have a credential about his address. So Bob requests such a credential from Alice and she issues it. In this scenario we also need a verifier, say, Jake, who will issue proof request and verify credentials.
...
- As a prerequisites credential schema and revocation registry definition are created
- Bob creates credential proposal
- Bob sends the proposal to Alice
- Alice creates credential offer based on Bob's proposal
- Alice sends the offer to Bob and he creates a credential request based on the offer
- Bob sends “request-credential” to Alice
- Alice receives it and creates the credential with the requested valued from Bob
- Alice wraps the credential in an “issue-credential” message and sends it back to Bob
- Bob receives the message and finalizes the credential. The “finalize-credential“ command returns blinded signature credential which can be used for creating presentations of it with zero knowledge proofs
- Jake (Verifier) issues a proof request, asking to reveal some of the credential attributes
- Jake sends the proof request message to Bob
- Bob receives the proof request from Jake and creates a verifiable presentation of Alice issued credential for him and sends it back to Jake
- Jake receives the verifiable presentation from Bob and can check the verification with the verify_proof command
- Jake gets the result of verification back and sees if the proof is valid

![State diagram](assets/bbs_issue_credential_and_exchange_00.drawio.png)

Above scenario will be using [`vade-evan-bbs`] for JWT credential creation and signing, [`vade-didcomm`] for [`issue credential protocol`] and didcomm communication, [`vade-evan-cli`] for combining all plugins and using them seamlessly. The flow in this scenario is one way to traverse through the [`issue credential protocol`] flow, as pictured in the illustrations. Paths not relevant to the example described here have been brightened up a bit to keep the focus on the actually traversed paths.
...
"schemaName":"test_schema",
"description":"Test description",
"properties":{"test_property_string":{"type":"string"}},
"requiredProperties":["test_property_string"],
"allowAdditionalProperties":false,
"issuerPublicKeyDid":"did:evan:testcore:0x6240cedfc840579b7fdcd686bdc65a9a8c42dea6#key-1",
"issuerProvingKey":"30d446cc76b19c6eacad89237d021eb2c85144b61d63cb852aee09179f460920"
}'
...
To create a revocation registry we use the command `vc_zkp create_revocation_registry_definition` and has the parameters `method`, `options` and `payload`
...
## Creating a credential proposal (Bob)
...
![State diagram](assets/bbs_issue_credential_and_exchange_01.drawio.png)
## Creating a credential offer (Alice)

This step is on the issuer side of the flow. In this step he takes the proposal of the Holder (Bob) and creates an credential offer based on the proposal.
...
![State diagram](assets/bbs_issue_credential_and_exchange_02.drawio.png)
## Bob requests a credential from Alice

This step is on the holder side of the flow. Now the holder (Bob) is able to create a concrete credential request with the values he wants to have issued by the issuer (Alice)
...
The second element in the returned array contains the signature blinding which is needed for finishing the credential signature returned by the issuer (Alice).

![State diagram](assets/bbs_issue_credential_and_exchange_03.drawio.png)
## Alice issues a credential for Bob

When Alice now receives the credential request from the Holder (Bob), she is now able to issue/create a new credential for him. For that she needs the following parameters:
...
```json
 {
	"unsignedVc": {
		"@context": [
			"https://www.w3.org/2018/credentials/v1",
			"https://schema.org", "https://w3id.org/vc-revocation-list-2020/v1"
		],
		"id": "uuid:any",
	...
	"issuerPublicKeyId": "did:evan:testcore:0x0d87204c3957d73b68ae28d0af961d3c72403906#key-1",
...
This credential is now sent back to the Holder (Bob) who must finalize the credential with his signature blinding value

![State diagram](assets/bbs_issue_credential_and_exchange_04.drawio.png)
## Finishing a credential (Bob)

When the holder (Bob) receives his BBS+ credential he needs to verify if the signature is mapped for his received signature blinding
...
![State diagram](assets/bbs_issue_credential_and_exchange_05.drawio.png)
## Creating a Proof request (Verifier)

When now a Verifier (Jake) wants to check that a Holder (Bob) has a credential for a given schema and wants to have the value `test_property_string` revealed from the Holder, he must create a proof request and send it to the Holder:
...
This returns the proof request which can be presented to the Holder (Bob):
...
![State diagram](assets/bbs_issue_credential_and_exchange_06.drawio.png)
## Creating a Verifiable Presentation (Holder/Prover)

When the Holder (Bob) now receives the Proof Request from the Verifier (Jake), he now creates a verifiable presentation out of the proof request
...
This would now return the verifiable presentation for the proof request and can be now sent back to the Verifier (Jake):
...
![State diagram](assets/bbs_issue_credential_and_exchange_07.drawio.png)
## Verifying a Verifiable Presentation (Verifier)

When the Verifier (Jake) now wants to check a received verifiable presentation against his created proof request he can use the following command:
...
![State diagram](assets/bbs_issue_credential_and_exchange_08.drawio.png)
...
[`vade-evan-cli`]: /
[`vade-evan-bbs`]: /docs/plugins/credentials/bbs
